document.addEventListener("DOMContentLoaded", () => {
  ...
  window.showSeatDetail = function (roomIndex) {
    ...
    const modal = new bootstrap.Modal(
      document.getElementById("seatDetailModal")
    );
    modal.show();
  };
  ...
  function generateEditableSeatLayout(room) {
    ...
    room.seatLayout.forEach((row, rowIndex) => {
      ...
      row.forEach((seatData, seatIndex) => {
        ...
        seat.title = `Row ${String.fromCharCode(65 + rowIndex)}, Seat ${
          seatIndex + 1
        }`;
...
document.addEventListener("DOMContentLoaded", function () {
  document.querySelectorAll(".toggle-active-btn").forEach((btn) => {
    btn.addEventListener("click", function () {
      const cinemaRoomId = this.getAttribute("data-id");

      fetch(
        `/admin/room-management/toggle-active?cinemaRoomId=${cinemaRoomId}`,
        {
          method: "GET",
        }
      )
        ...
        .then((result) => {
          if (result === "success") {
            location.reload(); // Reload lại để cập nhật trạng thái
          } else {
            alert("Error: " + result);
          }
        })
