...
// API base URL (configure this based on your Swagger URL)
const API_BASE_URL = 'http://localhost:8081'; // Matches your provided URL
...
function populateMovies() {
    ...
    fetch(`${API_BASE_URL}/api/employee/movies`, {
        method: 'GET',
        headers: {
...
function populateShowtimes(movieId, dateStr) {
    ...
    if (movieId && dateStr) {
        fetch(`${API_BASE_URL}/api/employee/showtimes?movieId=${movieId}&date=${dateStr}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
...
function populateSeats() {
    ...
    if (bookingData.movieId && bookingData.showDateId && bookingData.scheduleId) {
        fetch(`${API_BASE_URL}/api/booking/seats?movieId=${bookingData.movieId}&showDateId=${bookingData.showDateId}&scheduleId=${bookingData.scheduleId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            }
...
function populateCombos() {
    ...
    comboList.innerHTML = "";
    fetch(`${API_BASE_URL}/api/booking/combos`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            // Add authentication header if required by Swagger (e.g., 'Authorization': 'Bearer your-token-here')
...
async function submitBooking() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/employee/booking/create`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                // Add authentication header if required by Swagger (e.g., 'Authorization': 'Bearer your-token-here')
...
// Cải thiện Apply promotion code dựa trên logic từ payment.js
async function applyPromotionCode(promoCode, originalAmount, promoError) {
    try {
        const response = await fetch(`${API_BASE_URL}/api/booking/apply-promotion`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        bookingData.promotionCode = data.promotionCode;
...
async function checkMemberStatusByEmail(email) {
    ...
    applyEmailBtn.textContent = "Searching...";
    memberError.innerHTML = '<div class="alert alert-info" role="alert">Searching for member...</div>';

    try {
        const response = await fetch(`${API_BASE_URL}/api/employee/points?email=${encodeURIComponent(email)}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            }
        ...
        memberError.innerHTML = '<div class="alert alert-danger" role="alert">Account does not exist!</div>';
        bookingData.memberPoints = 0;
        bookingData.accountId = null;
        updatePointsDisplay();
    }
...
function applyPointsDiscount() {
    ...
    const discountPercentage = ((actualDiscountAmount / priceState.originalPrice) * 100).toFixed(1);
    console.log(`Applied ${usedPoints.toLocaleString('vi-VN')} points for ${actualDiscountAmount.toLocaleString('vi-VN')}đ discount (${discountPercentage}% of total)`);
