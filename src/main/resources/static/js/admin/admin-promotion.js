document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addPromotionForm) {
        ...
        addPromotionForm.addEventListener('submit', function (event) {
            ...
            if (hasPercentDiscount && (maxAmountForPercentDiscount < 1000) || (maxAmountForPercentDiscount > 100000)) {
                event.preventDefault();
                alert('Số tiền giảm tối đa phải từ 1.000đ đến 100.000đ.');
                return;
            }

            if (hasDirectDiscount && (discountAmount < 1000) || (discountAmount > 100000)) {
                event.preventDefault();
                alert('Số tiền giảm trực tiếp phải từ 1.000đ đến 100.000đ.');
                return;
            }
