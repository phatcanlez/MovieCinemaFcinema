...
public interface MovieScheduleRepository extends JpaRepository<MovieSchedule, MovieScheduleId> {
        ...
        /*
         * Lấy toàn bộ lịch chiếu trong một phòng (không giới hạn ngày)
         * Trả về DTO gồm: giờ chiếu, tên phim, thông tin phim, ngày chiếu
         */
        @Query("""
                        SELECT new com.example.projectwebmovie.dto.MovieScheduleForShowToAdminDTO
                        (
                            ...
                            ms.movie.movieNameVn,
                            ms.movie,
                            ms.showDates.showDate
                        )
                        FROM MovieSchedule ms
                        WHERE ms.cinemaRoom.cinemaRoomId = :cinemaRoomId
                        """)
        List<MovieScheduleForShowToAdminDTO> findSchedulesByRoom(
                        @Param("cinemaRoomId") Integer cinemaRoomId);
        ...
        // Tìm lịch chiếu trong một phòng theo ngày chiếu
        @Query("""
                            SELECT TIME(m.schedule.scheduleTime)
                            FROM MovieSchedule m
                            WHERE m.cinemaRoom.cinemaRoomId = :roomId
                              AND m.showDates.showDate = :showDate
                        """)
        List<LocalTime> findScheduleTimesByRoomIdAndShowDate(@Param("roomId") int roomId,
                        @Param("showDate") LocalDate showDate);
