...
function updatePaymentTimer() {
    ...
    console.log('🚀 ~ updatePaymentTimer ~ elapsedTime:', elapsedTime)
...
function updatePriceSummary() {
    ...
    if (totalAmount) {
        const grandTotal = bookingDataFromSession.totalPrice || bookingDataFromSession.totalPrice || 0;
...
function initPromoCode() {
    ...
    if (applyPromoBtn && promoCodeInput) {
        applyPromoBtn.addEventListener('click', function () {
            ...
            let currentTotal = priceState.currentPrice || 0;
            ...
            fetch(`/api/booking/apply-promotion`, {
                ...
                .then(data => {
                    ...
                    document.getElementById('totalAmount').textContent = `${data.finalAmount.toLocaleString('vi-VN')}đ`;
                ...
                `;
                    ...
                    // Lưu promotionId vào bookingDataFromSession
                    bookingDataFromSession.promotionId = data.promotionId;
                    sessionStorage.setItem('bookingData', JSON.stringify(bookingDataFromSession));

                    // Lưu thông tin giá gốc để có thể khôi phục khi hủy mã
                    bookingDataFromSession.originalPrice = data.originalAmount;

                    // Cập nhật tổng tiền
                    bookingDataFromSession.totalPrice = data.finalAmount;
                    ...
                    cancelBtn.addEventListener('click', function () {
                        ...
                        cancelBtn.addEventListener('click', function () {
                            ...
                            document.getElementById('totalAmount').textContent = `${data.originalAmount.toLocaleString('vi-VN')}đ`;
                            bookingDataFromSession.totalPrice = data.originalAmount;
            ...
            // Kiểm tra nếu đã có promotion áp dụng trước đó (khi tải lại trang)
            if (bookingDataFromSession.promotionId) {
                ...
                // Hiển thị lại dòng khuyến mãi
                const originalPrice = bookingDataFromSession.originalPrice || 0;
                const finalPrice = bookingDataFromSession.totalPrice || 0;
                ...
                cancelBtn.addEventListener('click', function () {
                    ...
                    if (bookingDataFromSession.originalPrice) {
                        document.getElementById('totalAmount').textContent =
                            `${bookingDataFromSession.originalPrice.toLocaleString('vi-VN')}đ`;
                        bookingDataFromSession.totalPrice = bookingDataFromSession.originalPrice;
                    }
...
function updateMaxUsablePoints(points) {
    ...
    try {
        ...
        currentTotal = bookingDataFromSession.totalPrice || 0;
    }
...
function applyUserPoints() {
    ...
    let currentTotal = priceState.originalPrice - priceState.voucherDiscount;
    ...
    const discountPercentage = ((discountAmount / (priceState.originalPrice - priceState.voucherDiscount)) * 100).toFixed(1);

    // Hiển thị thông báo thành công với thông tin chi tiết
    alertSystem.success(`Đã sử dụng ${maxPoints.toLocaleString('vi-VN')} điểm để giảm ${discountAmount.toLocaleString('vi-VN')}đ (${discountPercentage}% của đơn hàng)`, 5000);
}
...
function updatePointsDetailDisplay(usedPoints) {
    ...
    console.log(`Session Storage after applying points:`, bookingDataFromSession);
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Khôi phục trạng thái từ session nếu có
    if (bookingDataFromSession.originalPrice) {
        priceState.originalPrice = bookingDataFromSession.originalPrice;

        // Nếu có khuyến mãi voucher
        if (bookingDataFromSession.promotionId) {
            priceState.voucherDiscount = priceState.originalPrice - (bookingDataFromSession.totalPrice || priceState.originalPrice);
            if (bookingDataFromSession.usedPoints) {
                // Nếu cũng có điểm được sử dụng, điều chỉnh giá trị giảm giá voucher
                const pointsDiscount = bookingDataFromSession.usedPoints * 10;
                priceState.voucherDiscount -= pointsDiscount;
                priceState.pointsDiscount = pointsDiscount;
            }
        }
        // Nếu chỉ có điểm được sử dụng
        else if (bookingDataFromSession.usedPoints) {
            priceState.pointsDiscount = bookingDataFromSession.usedPoints * 10;
        }
    ...
    console.log('Initial price state:', priceState);
