...
public class SeatService {
    ...
    public void updateSeatMap(Integer cinemaRoomId, List<SeatForCreateRequestDTO> seatList) {
        ...
        seatJdbcRepository.updateSeatsBatch(entityList);
    }
    ...
    public void createScheduleSeatsForSchedules(List<MovieSchedule> schedules) {
        ...
        for (Map.Entry<String, List<MovieSchedule>> entry : grouped.entrySet()) {
            ...
            // 1. Load danh sách ghế đang có trong phòng (SeatType luôn mới nhất)
            // Lấy và sắp xếp danh sách ghế
            List<Seat> seats = seatRepository.findActiveSeatsByRoomId(roomId);
            ...
            // 3. Xoá ghế thuộc suất chiếu bị huỷ
            ...
            // 6. Lưu tất cả một lần
            if (!toInsert.isEmpty()) {
                seatJdbcRepository.insertSchedulesSeatsBatch(toInsert);
            }
        }
    }
