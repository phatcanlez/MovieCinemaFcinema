...
public class AdminController {
    ...
    @GetMapping("/room-management/toggle-active")
    @ResponseBody
    public String toggleRoomActiveStatus(@RequestParam Integer cinemaRoomId) {
        try {
            cinemaRoomService.toggleRoomActiveStatus(cinemaRoomId);
            return "success";
        } catch (Exception e) {
            logger.error("Error toggling room active status: {}", e.getMessage(), e);
            return "error: " + e.getMessage();
        }
    }
    ...
    public String generateRoomSeats(@RequestParam int maxRow,
        ...
        if (maxRow > 20) {
            model.addAttribute("errorSeatQuantityMess", "The number of row must be lower than or equal to 20.");
            check = false;
        }
        if (maxCol > 12) {
            model.addAttribute("errorSeatQuantityMess", "The number of column must be lower than or equal to 12.");
            check = false;
        }
        if (check) {
    ...
    public String showShowtimes(Model model,
        ...
        List<CinemaRoom> rooms = movieService.getActiveCinemaRooms();
