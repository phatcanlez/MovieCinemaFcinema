...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.openScheduleModal').forEach(btn => {
        ...
        btn.addEventListener('click', function () {
            ...
            const minFromDateValid = new Date(minFromDate) < today ? tomorrow : new Date(minFromDate);
            setupDateMinMaxValidation(minFromDateValid, maxToDate);
            fp.set('minDate', minFromDateValid);
        ...
        document.querySelectorAll('input[name="scheduleRoomMap"]').forEach(cb => {
            cb.addEventListener('change', function () {
                ...
                if (this.checked) {
                    ...
                    const selectedStartMins = timeToMinutes(selected.startTime) - 10 / 60;
                    const selectedEndMins = timeToMinutes(selected.endTime) + 10 / 60;
                    let delay = 0
                    ...
                    if (selectedSchedules.length === 0) {
                        ...
                        allCheckboxes.forEach(item => {
                            ...
                            const isConflict = selectedSchedules.some(sel => (
                                otherEnd > (sel.startMins - breakTime) &&
                                otherStart < (sel.endMins + breakTime)
                            ));
