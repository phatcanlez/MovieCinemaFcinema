...
public class CinemaRoomService {
    ...
    /**
     * Nếu phòng chưa có lịch chiếu thì cho phép đổi tên và id, đồng thời update luôn.
     * @param roomId id phòng chiếu
     * @param newRoomName tên phòng mới
     * @param movieScheduleService service để kiểm tra
     * @return true nếu update thành công (chưa có lịch chiếu), false nếu đã có lịch chiếu
     */
    @Transactional
    public int updateRoomInfoIfNoSchedule(Integer roomId, String newRoomName) {
        if (movieScheduleService.existsMovieScheduleByRoomId(roomId)) {
            return -1;
        }
        if(this.isExistNameRoom(newRoomName)) {
            return -2; // Không cho đổi tên nếu đã có phòng khác trùng tên
        }
        CinemaRoom room = cinemaRoomRepository.findById(roomId)
                .orElseThrow(() -> new RuntimeException("Phòng chiếu không tồn tại"));
        if (newRoomName != null && !newRoomName.trim().isEmpty()) {
            room.setCinemaRoomName(newRoomName.trim());
        }
        cinemaRoomRepository.save(room);
        return 1;
    }

    public void toggleRoomActiveStatus(Integer cinemaRoomId){
